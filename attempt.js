...
module.exports.attempt = function ( available, allowed, preffered )
{
  ...
  {
    let closestRight = interSection.filter( ( num ) => num <= preffered[ preffered.length - 1 ] );
    if( ( interSection[ interSection.length - 1 ] > preffered[ preffered.length - 1 ] )
  ...
  //------------------ //state only uniqe elements of the result array//--------------------------------------------

  result = Array.from( new Set( result ) );
  if( Math.max( ... allowed ) <= preffered[ 0 ] )
